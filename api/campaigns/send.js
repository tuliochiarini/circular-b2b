...
const DEFAULT_FROM = 'Circular B2B <contato@circularb2b.eco.br>';
const DEFAULT_REPLY_TO = 'comercial.circularb2b@gmail.com';

function isAuthorized(req) {
  const expected = process.env.CAMPAIGN_API_TOKEN;
...
async function sendMessage(message, campaignId) {
  const response = await fetch('https://api.resend.com/emails', {
    ...
    headers: {
      ...
      'Idempotency-Key': `circular-${campaignId}-${message.externalId}`,
    },
    body: JSON.stringify({
      ...
      tags: [
        ...
        { name: 'lead', value: message.externalId.trim() },
      ],
    }),
  });
...
export default async function handler(req, res) {
  ...
  if (!process.env.RESEND_API_KEY || !process.env.CAMPAIGN_API_TOKEN) {
