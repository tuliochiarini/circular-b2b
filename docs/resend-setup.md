...
3. Adicionar na Vercel, somente no servidor, `RESEND_API_KEY` e `CAMPAIGN_API_TOKEN`.
4. Configurar `RESEND_FROM` e `RESEND_REPLY_TO` conforme `.env.example`.
5. Fazer primeiro um `dryRun` e depois um envio real para uma conta interna.

## Proteções incluídas

- autenticação por token privado;
